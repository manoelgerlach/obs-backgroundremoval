...
try {
    ...
    foreach ($expected in @(
            'obs-backgroundremoval — Windows DirectML fork',
            'Version 0.0.0',
            'Unofficial modified fork',
            'not an official upstream release',
            'Fork maintainer and modifications (2026): Manoel Gerlach',
            'Fork Repository',
            'Original / Upstream Project',
            'Copyright © 2021–2026 Roy Shilkrot',
            'Copyright © 2023–2026 Kaito Udagawa',
    ...
    if ($toggle.Current.ToggleState -ne [System.Windows.Automation.ToggleState]::Off) {
        throw 'Update-notification checkbox should initially be clear.'
    }
    if ($checkbox.Current.IsEnabled) {
        throw 'Update-notification checkbox should be disabled when no fork feed is configured.'
    }
